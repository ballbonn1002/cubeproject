...
public class MyTimesheetAction extends ActionSupport {
	...
	public String SearchMyTimeSheet() {
		try {
			User ur = (User) request.getSession().getAttribute("onlineUser");
			String search_user = ur.getId();
	...
	public Map<String, Object> TimeCalculate(List<Map<String, Object>> TimeList, List<Map<String, Object>> leavelist,List<Map<String , Object>> holiday)
		...
		for (Map<String, Object> map : TimeList) {
			...
			if (day_of_work.equalsIgnoreCase("sat") || day_of_work.equalsIgnoreCase("sun")) {
				...
				}else if((ot1start <= OT_in_min && OT_in_min <= ot1end) && (ot2start <= OT_out_min && OT_out_min <= ot2end)) {
					x1_ot_min_count += (OT_out_min - ot2start);
					x3_ot_min_count += (ot1end - OT_in_min);
					
				}else if((ot1start <= OT_in_min && OT_in_min <= ot1end) && (ot3start <= OT_out_min && OT_out_min <= ot3end)) {
					x3_ot_min_count += (ot1end - OT_in_min);
				...
				if((ot2start <= OT_in_min && OT_in_min <= ot2end) && (ot2start <= OT_out_min && OT_out_min <= ot2end)) {
					x1_ot_min_count += (OT_out_min - OT_in_min);
					
				}else if((ot2start <= OT_in_min && OT_in_min <= ot2end) && (ot3start <= OT_out_min && OT_out_min <= ot3end)) {
					x1_ot_min_count += (ot2end - OT_in_min);
					x3_ot_min_count += (OT_out_min - ot3start);
					
				}else if((ot2start <= OT_in_min && OT_in_min <= ot2end) && (ot1start <= OT_out_min && OT_out_min <= ot1end)) {
					x1_ot_min_count += (ot2end - OT_in_min);
				...
				}else if((ot3start <= OT_in_min && OT_in_min <= ot3end) && (ot1start <= OT_out_min && OT_out_min <= ot1end)) {
					 x3_ot_min_count += (ot3end - OT_in_min);
					 x3_ot_min_count += (OT_out_min - ot1start);
					
				}else if((ot3start <= OT_in_min && OT_in_min <= ot3end) && (ot2start <= OT_out_min && OT_out_min <= ot2end)) {
					x3_ot_min_count += (ot3end - OT_in_min);
				...
				if ((ot1start <= timein_min && timein_min <= ot1end) && (ot1start <= timeout_min && timeout_min <= ot1end)) {
					x3_ot_min_count += (timeout_min - timein_min);
					
				}else if((ot1start <= timein_min && timein_min <= ot1end) && (ot2start <= timeout_min && timeout_min <= ot2end)) {
					x1_ot_min_count += (timeout_min - ot2start);
					x3_ot_min_count += (ot1end - timein_min);
					
				}else if((ot1start <= timein_min && timein_min <= ot1end) && (ot3start <= timeout_min && timeout_min <= ot3end)) {
					x3_ot_min_count += (ot1end - timein_min);
					x1_ot_min_count += (ot2end - ot2start);
				...
				if((ot2start <= timein_min && timein_min <= ot2end) && (ot2start <= timeout_min && timeout_min <= ot2end)) {
					x1_ot_min_count += (timeout_min - timein_min);
				
				}else if((ot2start <= timein_min && timein_min <= ot2end) && (ot3start <= timeout_min && timeout_min <= ot3end)) {
					x1_ot_min_count += (ot2end - timein_min);
					x3_ot_min_count += (timeout_min - ot3start);
					
				}else if((ot2start <= timein_min && timein_min <= ot2end) && (ot1start <= timeout_min && timeout_min <= ot1end)) {
					x1_ot_min_count += (ot2end - timein_min);
					x3_ot_min_count += (ot3end - ot3start);
				...
				}else if((ot3start <= timein_min && timein_min <= ot3end) && (ot1start <= timeout_min && timeout_min <= ot1end)) {
					 x3_ot_min_count += (ot3end - timein_min);
					 x1_ot_min_count += (timeout_min - ot1start);
					 
				}else if((ot3start <= timein_min && timein_min <= ot3end) && (ot2start <= timeout_min && timeout_min <= ot2end)) {
					x3_ot_min_count += (ot3end - timein_min);
					x3_ot_min_count += (ot1end - ot1start);
					x1_ot_min_count += (timeout_min - ot2start);
					//int a [] [] [] [] [] [] [] [] [] []= null;
				...
				}else if((ot2end <= OT_in_min && OT_in_min <= ot3start) && (ot1start <= OT_out_min && OT_out_min <= ot1end)) {
		...
		for (Map<String, Object> map : leavelist) {
			...
			if (Integer.valueOf(startdate.substring(3, 5)) == Integer.valueOf(enddate.substring(3, 5))) {
				...
				for (int i = (Integer.valueOf(startdate.substring(0, 2))); i <= Integer.valueOf(enddate.substring(0, 2)); i++) {
	...
	public String ExcelExportMyTimesheet() {
		try {
			User ex = (User) request.getSession().getAttribute("onlineUser");
			boolean daycheck = false;
			String userid = ex.getId();
			...
			for (int i = 7; i <= rowd; i++) {
				...
				for (String day : dayinlist) {
					if (daylist.get(i - 7).equals(day)) {
						...
						if(parsdayofweek.format(date1).equalsIgnoreCase("sat") || parsdayofweek.format(date1).equalsIgnoreCase("sun")) {
				...
				for (String leaveday : leavelistStartDate) {
					...
					if (leaveday.equals(daylist.get(i - 7))) {
						...
						if (Integer.valueOf(leaveday.substring(3, 5)) < Integer
							...
							if (leavelistEndDate.get(leavelistStartDate.indexOf(leaveday)).equals(daylist.get(i - 7))) {
								// past to now
								...
								for (int c = 0; c < leavedaynumcount; c++) {
									...
									if(day_of_week_format.equalsIgnoreCase("sat") || day_of_week_format.equalsIgnoreCase("sun")) {
											cell = row.getCell(0);
											cell.setCellValue(daylist.get(i - 7 - c));
											cell.setCellStyle(Holidaystyle);
			...
			cell.setCellValue(daycount-(holidaycount+weekendcount+workdaycount+Integer.parseInt(TimeCal.get("leave_day_count").toString())));
