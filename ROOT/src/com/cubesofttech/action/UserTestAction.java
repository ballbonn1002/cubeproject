...
import com.cubesofttech.dao.ApproverDAO;
...
///////////
	...
	public String openEdit() {
		try {
			...
			request.setAttribute("userLists", userListJSON);
			
			log.debug("userListJSON: "+userListJSON);
			log.debug("useridEdit: "+userId);
			String approverListJSON = approverDAO.apprListById(userId);
			request.setAttribute("approverLists", approverListJSON);
			log.debug("approver==="+approverListJSON);
			
			
			//String approverListAll = approverDAO.apprListJSON();
			//request.setAttribute("approverListsAll", approverListAll);//ไม่ได้ใช้มั้ง
	...
	public String performEdit() {
		...
		try {
			...
			if (fileUpload != null) {
				...
				request.setAttribute("selectUser", selectUser);
				//log.debug("today2>> "+selectUser); //test
				...
				String approverListJSON = approverDAO.apprListById(userId);
				request.setAttribute("approverLists", approverListJSON);
				log.debug("approverE==="+approverListJSON);

				List<Map<String, Object>> approverList = approverDAO.findApproverById(String.valueOf(userId));
				log.debug("size approverList : "+approverList.size());
				log.debug("approverList==="+approverList);

				String approverListAll = approverDAO.apprListJSON();
				request.setAttribute("approverListsAll", approverListAll);
				
				String getappr = approver.getApprId();
		        String[] arrOfStr = getappr.split("/"); 
		        log.debug("arrOfStr :"+arrOfStr[0]);
		       // log.debug("arrOfStr :"+arrOfStr.length);
		        int count = 0;
		        if(arrOfStr[0].equals("deleteAll")) {
		        	log.debug("<<<Delete>>>");
		        	log.debug("userid IF>>>: "+userId);
		        	
		        	for(int i = 0 ; i < approverList.size() ;i++) {
		        		int id = ((Integer)approverList.get(i).get("id"));
		        		
		        		Approver approver = new Approver();
		        		approver = approverDAO.findById(id);
		    			log.debug(approver);
		    			approverDAO.delete(approver);
				        	...
				        	for (int idappr = 0 ; idappr < arrOfStr.length-1 ;idappr++ ) {
								if(count<approverList.size()) {
									log.debug("<<<Edit>>>");
									int id = ((Integer)approverList.get(idappr).get("id"));//loop ตาม size
									log.debug("id: "+id);
									Approver approvers = approverDAO.findById(id);						
									List<Map<String, Object>> apprName = userDAO.apprNameById(arrOfStr[idappr]);
	
									log.debug("apprName :"+apprName);
									String name = (String) apprName.get(0).get("name");
									
									log.debug("test2 :"+arrOfStr[idappr]);
									approvers.setApprId(arrOfStr[idappr]);
									approvers.setApprName(name);
									
									approverDAO.update(approvers);
								}					
								count++;
				        	}
				        }
...
//			 String tempRow = request.getParameter("tempRow");
//			 log.debug("tempRow: "+tempRow);
				// Approver approver = new Approver();
				...
				for (int i = 1; i < 4; i++) {
					for (int j = 1; j < 4; j++) {
						...
						Approver approver =new Approver();
						String appr = request.getParameter("approver"+i+"-"+j);
						log.debug("approver"+i+"-"+j+":"+appr);

						if(appr!=null) {
							List<Map<String, Object>> apprName = userDAO.apprNameById(appr);
							String name = (String) apprName.get(0).get("name");
							log.debug("Name"+i+"-"+j+":"+name);
							approver.setId(approverDAO.getMaxId() + 1);
							approver.setUserId(userId);
							approver.setApprId(appr);
							approver.setApprName(name);
							approver.setApprNo(j-1);
							approver.setRow(i-1);
	...
	public void editOther() throws Exception {
		...
		//List<Map<String, Object>> cubesoftUsers = userDAO.Query_Userlist2();
		 ...
		 int count = 0;
		 String temp[] = appr_id.split(", ");
		 
 		 for(int i=0 ; i<temp.length;i++) {	 
 			 List<Map<String, Object>> apprName = userDAO.apprNameById(temp[i]);
			 String name = (String) apprName.get(0).get("name");
 			 log.debug("appr_id: "+appr_id+":"+name);
			 log.debug("tepm: "+temp[i]);
			 log.debug("userId: "+userId); 
			 
			 approverDAO.findApprById(userId,temp[i],name,(row-1),i);
			 
		 count++;
		 }
		 approverDAO.deleteById(userId,row-1,count);
		 log.debug("FFFFFF"); 
		} catch (Exception e) {
			e.printStackTrace();
		...
		public void deleteRow() throws Exception {
			try {
			 ...
			 int rowStop =  Integer.parseInt(request.getParameter("rowStop"));
			 log.debug("userId: "+userId+":"+rowStart+">>"+rowStop); 
			 
			 approverDAO.deleteByRow(userId,rowStart-1);
			 for(int i = rowStart-1 ;i<rowStop ; i++) {
				 log.debug("edit row:"+i); 
				 approverDAO.UpdateByRow(userId,i);
