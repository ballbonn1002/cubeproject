...
<div class="portlet light bordered">
	...
	<div class="portlet-body">
		...
		<form name="formFAQ" id="formFAQ" method="POST"
			...
			<input name="faq_id" type="hidden" value="${faqs.faq_id}">
			<div class="form-body">
							...
							<label class="control-label">Category</label> <select
								...
								<c:forEach var="faq_cat" items="${faqCategoryList}">
									...
									<option class="category_list_option"
										<c:if test="${fn:containsIgnoreCase(faqs_category.faq_cat_name,faq_cat.faq_cat_name)}">
					...
					<div class="col-md-2 col-xs-7">
						<div class="form-group">
							...
							<label class="control-label">Status</label> <select
								id="status_list_select" name="faq_status_name"
								class="form-control" data-placeholder="Choose a Status"
								tabindex="1">
								<c:forEach var="faq_status" items="${faqStatusList}">

									<option class="status_list_option"
										<c:if test="${fn:containsIgnoreCase(faqs_status.faq_status_name,faq_status.faq_status_name)}">
											<c:out value=" selected " />
										</c:if>
										value="${faq_status.faq_status_name}">${faq_status.faq_status_name}</option>
								</c:forEach>
										...
										<c:if test="${user.enable == 1 }">
											<c:if test="${logonUser == nulll }">
												<option value="${user.id}" id="${user.id}"
													<c:if test="${fn:containsIgnoreCase(user.id,faqs.author)}"><c:out value="selected=selected"/>
	...
	$(document)
			.ready(
					function() {
						...
						$('#summernote').summernote('code', '${faqs.details}');
					});
