				...
				if(item.Sender_id.toLowerCase() == Owner_id){
					...
					if(item.file_message_status == '1'){
						...
						if(video.toUpperCase() === ".MP4")
						{
							...
							html+= '<a href="#" name = "openvid" id ="'+item.file_path+'"data-toggle="modal" data-target="#exampleModalvid">'+item.file_name+'</a>';
							//$('#watchvid').attr('src', item.file_path);
						}
						else
						{
							html+=' <i class="fa fa-file"></i>&ensp;';	
							html+= '<a href="'+item.file_path+'" target="_blank">'+item.file_name+'</a>';
						}
					...
					html+='     <small style="float:right; padding-top: 5px" class="mf-date"><i class="fa fa-clock-o"></i> '+item.Date_time_Message+''; 
					...
					html+='       <small style="float:left; padding-top: 5px" class="mf-date"><i class="fa fa-clock-o"></i> '+item.Date_time_Message+'</small>';
					if(item.like_status == '1'){
...
function checkcreategroup(){
	...
	if(name.trim() != "" && description.trim() != "" && length > 0){
		...
		success:function(data)
        {
			...
			if(current_length != length){
				if(topload == true){
					...
					toBottom();
					//console.log('false topload to tom');
				}else {
					toBottom();
					//console.log('false to tom');
				}
...
function getUserComparetoMember (data){
	...
	var htmls = "";
	json.forEach((item) => {
		if(!(selectedsmember.includes(item.id.toLowerCase()))){
	...
	json.forEach((item) => {
		if(item.file_message_status == "1"){
			...
			if(!(split.toUpperCase() === ".MP4")) {
				htmlsqw += '<div><a href="'+item.file_path+'" target="_blank">'+item.file_name+'</a></div>';
			}			
		}
	});
	$('#file-file').html('');
...
function show_video(data){
	...
	json.forEach((item) => { 
		if(item.file_message_status == "1"){
			...
			var split = item.file_path.substr(item.file_path.lastIndexOf("."),(item.file_path).length);
			if(split.toUpperCase() === ".MP4") {
				htmlss += '<div><a href="#" name = "openvid" id ="'+item.file_path+'"data-toggle="modal" data-target="#exampleModalvid"><video src="'+item.file_path+'"></video></a></div>';
			}
...
function searchmemberlist(){
	var searchtxt = $('#searchmembertext').val();
			...
			jsondata.forEach((item) => {
					if(item.favorite_mark == "1"){
    		 		...
    		 		if(item.Group_Description != null || item.Group_Description == ""){
    		 			html += '  <small class="list-group-item-text c-gray">'+item.Group_Description+'</small>';
    		 		}else{
    		 			html += '  <small class="list-group-item-text c-gray">Chat Group Description</small>';
    		 		}
...
if(theme == '1'){
			...
			$('#sticker').css('background-color' , '#2f3136');
			...
			$('#sticker').css('background-color' , '#fff');
			...
			$('#sticker').css({background: "radial-gradient(circle at 0% 0%, #373b52, #252736 51%, #1d1e26)"}); 
...
function checkRefresh(Chattype){
	...
	function showfoundresult(data){
		...
		var Owner_id = $('#Owner_chat_id').val().toLowerCase();
		var reciever_avartar =$('#reciever_avatar').attr('src');
		var textsearch = $('#findmessageinchat').val().trim();
		var html = "";
		jsondata.forEach((item)=>{
			if(item.Sender_id.toLowerCase() == Owner_id){
					html+='<div class="message-feed right" >';  
...
$(document).on('keypress keyup keydown blur','#Message_text', function (e) {
	...
	if(e.type == "keyup" || e.type == "keydown"){
		...
		if(e.type == "keyup"){
			if (key == 86 && ctrl) { 
				char_count = 0;
				//console.log(message_input_text != "");
				//console.log(message_input_text == "");
				//console.log($(this).val() != "");
				//console.log((message_input_text != "") && ($(this).val() != ""));
				//console.log($(this).val().substr(message_input_text.lastIndexOf(message_input_text.slice(message_input_text.length - 1))+1,$(this).val().length-1));
			...
			if(e.keyCode == 8){
					...
					if((message_input_text.slice(message_input_text.length - 1)) == "&" && (message_input_text.slice(message_input_text.length - 2)) == "?&"){
						...
						if(message_input_text.lastIndexOf("&") != -1){
							//console.log(message_input_text.substring(message_input_text.lastIndexOf("&"),message_input_text.lastIndexOf("&")+2));
							if(message_input_text.substring(message_input_text.lastIndexOf("&"),message_input_text.lastIndexOf("&")+2) == "&#"){
								...
								emoji_length.pop();
								console.log(emoji_length);
								//message_input.slice(message_input.lastIndexOf("&"),-((message_input_text.length - 1) - message_input.lastIndexOf("&")));
								//console.log(message_input_text + "   intext");
								//console.log(message_input+ "  int");
							}
				...
				//console.log(message_input_text + " des ");
			}
			if($(this).val().trim() == ""){
					message_input_text = "";
					message_input = "";
	...
	*/if(e.type == "keypress"){
	...
	//	console.log(message_input_text + "   sssaa");
	}
...
$(document).on('click','a[name=addmember]',function(){
	...
	$.ajax({
		...
		data:{
			...
			$('#sortmemberdepart option[value="all"]').attr("selected",true);
		}
	});
});
		...
		data:{
			...
			var jsondata = JSON.parse(data);
		 	jsondata.forEach((item) => {
				...
				//$('#chageimagefile').val(null);
		 		$('#groupnameinput').val(item.Group_chat_Name);
		 		$('#groupdescriptioninput').val(item.Group_Description);
