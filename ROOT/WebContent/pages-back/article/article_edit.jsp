...
<div class="portlet light bordered">
	<div class="portlet-title">
		<div class="caption">
			<span class="caption-subject font-red sbold uppercase"> <i
				class="fa fa-edit"></i>&nbsp;Edit Article
			</span>
		</div>
		<div class="actions">
	...
	<div class="portlet-body">
		<!-- BEGIN FORM-->
		<form name="article_form" action="article_update" class="horizontal-form" method="POST" autocomplete="off" enctype="multipart/form-data">
								...
								<div class="col-md-12">
									...
									<div class="col-md-10">
										<input name="topic" type="text" class="form-control"
											placeholder="e.g. Top 10 Trending Technologies" value="${article.topic}" required>
									</div>
								</div>
							</div>
						</div>
								...
								<div class="col-md-12">
									...
									<c:choose>
										<c:when test="${Count.count eq article.articleTypeId}">
											<option class = "option_articleType" value="${articleType.article_type_id}" selected>${articleType.name}</option>
										</c:when>
										<c:otherwise>
											<option class = "option_articleType" value="${articleType.article_type_id}">${articleType.name}</option>
										</c:otherwise>
									</c:choose>
								</c:forEach>
											...
											<c:if test="${user.enable == 1 }">
												<c:if test="${logonUser == nulll }">
													<option value="${user.id}" id="${user.id}"
														<c:if test="${fn:containsIgnoreCase(user.id,article.userId)}"><c:out value="selected=selected"/>
									...
									<optgroup label="Disable">
										<c:forEach var="user" items="${cubeUser}">
											<c:if test="${user.enable == 0 }">
												<c:if test="${logonUser == nulll }">
													<option value="${user.id}" id="${user.id}"
														<c:if test="${fn:containsIgnoreCase(user.id,article.userId)}">
									...
									<div class="col-md-10 col-xs-12">
										<div>
											<input name="tags" type="text"
												class="form-control input-large" data-role="tagsinput" value="<c:forEach var="ArticleInTag" items="${ArticleInTag}"
									varStatus="Count">
...
<c:set var="imgCount" value = "0"></c:set>
									...
									<div class="fileinput-new thumbnail"
										...
										<c:if test="${arList.article_id == article.articleId}">
											<c:if test ="${imgCount == '0'}">
												<c:set var="imgCount" value = "1"></c:set>
												<c:choose>
													<c:when
	...
	<div class="form-actions action right">
				<div class="row">
					<div class="col-xs-12" style="text-align: center;">
						...
						<button onclick="cancel()" class="btn red">
...
<script>
	$(document)
			.ready(
					function() {
						...
						$('#summernote').summernote('code', '${article.detail}');
					});
	...
	function addArticleType() {
		//alert($("form[name='article_form']").serialize());
